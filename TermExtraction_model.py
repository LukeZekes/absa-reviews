  ...
  for entry in data:
    ...
    while i < len(spans):
      ...
      if spans[i][0] in aspect_tags: 
        spans[i][1] = int(TermClasses.ASPECT)
      elif spans[i][0] in opinion_tags:
        spans[i][1] = int(TermClasses.OPINION)
      else:
...
def load_model():
  ...
  filename = os.path.join(dirname, r'models\\ATE\\retrained\\100\\')
...
def create_model():
  ...
  model.add(keras.layers.Input(shape=(101, )))
  ...
  model.fit(x=vectorized_train_input, y=train_output, validation_data=(vectorized_test_input, test_output), epochs=15)
  model.save("models/ATE/retrained/100")
